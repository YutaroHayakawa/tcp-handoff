...
static int
tcp_get_queue_len(int sock, struct tcpho_tcp_state *state)
{
  int error, size;
...
static int
tcp_get_options(int sock, struct tcpho_tcp_state *state, struct tcp_info_sub *info)
{
  int error;
  uint32_t mss;
  uint32_t timestamp;
  ...
  state->mss = mss;
  state->send_wscale = info->tcpi_snd_wscale;
  state->recv_wscale = info->tcpi_rcv_wscale;
...
static int
tcp_set_options(int sock, const struct tcpho_tcp_state *state)
{
  int error;
  ...
  opts[1].opt_val = state->send_wscale + (state->recv_wscale << 16);
  opts[2].opt_code = TCPOPT_TIMESTAMP;
  opts[2].opt_val = 0;
  opts[3].opt_code = TCPOPT_MSS;
  opts[3].opt_val = state->mss;
  ...
  uint32_t tstamp = state->timestamp;
...
static int
tcp_get_window(int sock, struct tcpho_tcp_state *state)
{
  int error;
  ...
  state->snd_wl1 = window.snd_wl1;
  state->snd_wnd = window.snd_wnd;
  state->max_window = window.max_window;
  state->rcv_wnd = window.rcv_wnd;
  state->rcv_wup = window.rcv_wup;

  return 0;
}

static int
tcp_set_window(int sock, const struct tcpho_tcp_state *state)
{
  int error;
  struct tcp_repair_window window;

  window.snd_wl1 = state->snd_wl1;
  window.snd_wnd = state->snd_wnd;
  window.max_window = state->max_window;
  window.rcv_wnd = state->rcv_wnd;
  window.rcv_wup = state->rcv_wup;
  error =
...
static int
tcp_get_addr(int sock, struct tcpho_tcp_state *state)
{
  int error;
  ...
  state->self_addr = addr.sin_addr.s_addr;
  state->self_port = addr.sin_port;
  ...
  state->peer_addr = addr.sin_addr.s_addr;
  state->peer_port = addr.sin_port;

  return 0;
}

static int
tcp_set_addr(int sock, const struct tcpho_tcp_state *state)
{
  int error;
  struct sockaddr_in addr;

  addr.sin_family = AF_INET;
  addr.sin_addr.s_addr = state->self_addr;
  addr.sin_port = (uint16_t)state->self_port;
  error = bind(sock, (struct sockaddr *)&addr, sizeof(addr));
  if (error) {
    return errno;
  }

  addr.sin_addr.s_addr = state->peer_addr;
  addr.sin_port = (uint16_t)state->peer_port;
...
static int
tcp_get_queue(int sock, int qid, struct tcpho_tcp_state *state)
{
  int error;
  ssize_t ret;
  uint8_t *buf;
...
static int
tcp_set_queue(int sock, int queue, struct tcpho_tcp_state *state)
{
  int error;
  ...
  if (queue == TCP_RECV_QUEUE) {
    ...
    return send_queue(sock, TCP_RECV_QUEUE,
                      (const uint8_t *)state->recvq, state->recvq_len);
  }

  if (queue == TCP_SEND_QUEUE) {
    uint32_t len, ulen;

    ulen = state->unsentq_len;
    len = state->sendq_len - ulen;
    if (len) {
      error = send_queue(sock, TCP_SEND_QUEUE,
                         (const uint8_t *)state->sendq, len);
      if (error) {
        return error;
      }
    }
    ...
    if (ulen) {
      ...
      error = __send_queue(sock, TCP_SEND_QUEUE,
                           (const uint8_t *)state->sendq + len, ulen);
      if (error) {
        return error;
      }
...
static int
tcp_set_seq(int sock, int queue, struct tcpho_tcp_state *state)
{
  int error;
  uint32_t seq;
...
int
tcpho_tcp_export(int sock, struct tcpho_tcp_state *state)
{
  int error;
  struct tcp_info_sub info;

  if (sock < 0 || state == NULL) {
    return EINVAL;
  }
...
#define TRY(_funccall, _label)                                                 \
  ...
  TRY(tcp_get_queue_len(sock, state), err1);
  TRY(tcp_get_options(sock, state, &info), err1);
  TRY(tcp_get_window(sock, state), err1);
  TRY(tcp_get_addr(sock, state), err1);
  TRY(tcp_get_queue(sock, TCP_SEND_QUEUE, state), err1);
  TRY(tcp_get_queue(sock, TCP_RECV_QUEUE, state), err2);
...
int
tcpho_tcp_import(int sock, struct tcpho_tcp_state *state)
{
  int error;
...
#define TRY(_funccall, _label)                                                 \
  ...
  TRY(tcp_repair_start(sock), err0);
  TRY(tcp_set_seq(sock, TCP_SEND_QUEUE, state), err1);
  TRY(tcp_set_seq(sock, TCP_RECV_QUEUE, state), err1);
  TRY(tcp_set_addr(sock, state), err1);
  TRY(tcp_set_queue(sock, TCP_SEND_QUEUE, state), err1);
  TRY(tcp_set_queue(sock, TCP_RECV_QUEUE, state), err1);
  TRY(tcp_set_options(sock, state), err1);
  TRY(tcp_set_window(sock, state), err1);
